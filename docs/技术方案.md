...
下一阶段的演进重点需要优先放在以下能力上，顺序如下：

| 优先级 | 关键需求 | 说明 |
| --- | --- | --- |
| P0 | 阿里云 ACK/K8s 集群接入与 OpenSandbox 套件部署 | 接入阿里云服务的 K8s 集群，完成环境预检，一键部署完整 OpenSandbox 服务套件，并自动纳管为 OpenSandbox backend |
| P0 | 多 OpenSandbox 集群管理 | 平台管理员可注册多个 OpenSandbox 集群，查看健康和容量，并由调度策略选择合适集群承载新沙箱 |
| P0 | OpenSandbox 镜像管理与分发 | 平台管理员可手动上传镜像，上传完成后自动推送到各 OpenSandbox 集群镜像仓库，并跟踪每个集群的同步状态 |
| P1 | 项目大屏首页与文生图宣传资产 | 增加对外介绍和推销 OpenSandbox Plus 的大屏首页，结合文生图生成背景图、宣传图和功能示意图 |
...
- 多区域 backend 调度。
- 增加多 OpenSandbox backend / 集群管理、健康检查、容量状态和调度策略。
- 增加 OpenSandbox 镜像管理：手动上传镜像，上传完成后自动推送到各 OpenSandbox 集群镜像仓库，并跟踪同步状态。
- 增加阿里云 ACK/K8s 集群接入：支持接入阿里云服务的 K8s 集群，完成环境预检、一键部署完整 OpenSandbox 服务套件，并自动纳管为 OpenSandbox backend。
- 增加项目大屏首页：面向项目介绍和推广，展示核心价值、架构能力、使用流程和接入路径，并结合文生图能力生成背景图、宣传图和功能示意图。
